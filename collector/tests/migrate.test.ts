...
//
// The old DDL below is reproduced literally from the pre-product collector: PK
...
import { INSTANCE_A, INSTANCE_B, makeHeartbeat } from './fixtures';
...
describe('migration against a REAL old-schema database', () => {
  test('adds the product dimension, backfills every row as brokkr, and loses nothing', () => {
    ...
    const rows: SeedRow[] = [
      { instanceId: INSTANCE_A, day: '2026-07-07', payload: JSON.stringify(makeHeartbeat('brokkr', INSTANCE_A, '2026-07-07')) },
      { instanceId: INSTANCE_A, day: '2026-07-08', payload: JSON.stringify(makeHeartbeat('brokkr', INSTANCE_A, '2026-07-08')) },
      { instanceId: INSTANCE_B, day: '2026-07-08', payload: JSON.stringify(makeHeartbeat('brokkr', INSTANCE_B, '2026-07-08')) },
    ];
  ...
  test('a stored payload is carried across BYTE-IDENTICAL — the migration rewrites no document', () => {
    // There is one heartbeat schema, so there is nothing to convert at rest.
    // This is the invariant that replaced the retired v1 normalization pass:
    // not "conversion produced the right document", but "no conversion happens
    // at all". A rewrite reappearing here would be a silent data change.
    const path = tempDbPath();
    const stored = JSON.stringify(makeHeartbeat('brokkr', INSTANCE_A, '2026-07-07'));
    seedOldDatabase(path, [{ instanceId: INSTANCE_A, day: '2026-07-07', payload: stored }]);

    const db = new TelemetryDb({ path });
    try {
      expect(db.migration.rebuiltHeartbeats).toBe(true);
      const row = db.getHeartbeats(INSTANCE_A, LEGACY_PRODUCT)[0]!;
      expect(row.payload).toBe(stored);
  ...
  test('re-running is a no-op: no second rebuild, no second payload pass', () => {
    ...
    try {
      ...
      // Byte-identical, again: booting twice must not touch a stored document.
    ...
    try {
      expect(third.getHeartbeats(INSTANCE_A, LEGACY_PRODUCT)[0]!.payload).toBe(firstPayload);
  ...
  test('the collision the product column exists to prevent is gone', () => {
    ...
    try {
      ...
      db.recordHeartbeat(INSTANCE_A, 'saga', '2026-07-08', 1, JSON.stringify({ metrics: {} }), '2026-07-08');
  ...
  test('a naive ADD COLUMN upgrade is detected and completed, not mistaken for done', () => {
    ...
    try {
      ...
      expect(db.recordHeartbeat(INSTANCE_A, 'saga', '2026-07-09', 1, '{}', '2026-07-09').ok).toBe(true);
      expect(db.countHeartbeats(INSTANCE_A, 'saga')).toBe(1);
    } finally {
      db.close();
    }
  ...
  test('an unparseable stored payload is carried over verbatim, never dropped or repaired', () => {
    const path = tempDbPath();
    const good = JSON.stringify(makeHeartbeat('brokkr', INSTANCE_A, '2026-07-07'));
    seedOldDatabase(path, [
      { instanceId: INSTANCE_A, day: '2026-07-06', payload: 'not json at all' },
      { instanceId: INSTANCE_A, day: '2026-07-07', payload: good },
    ...
    try {
      ...
      expect(rows[1]!.payload).toBe(good);
  ...
  test('an upgraded database gets first_report_day derived from its surviving heartbeats', () => {
    ...
    seedOldDatabase(path, [
      { instanceId: INSTANCE_A, day: '2026-07-07', payload: JSON.stringify(makeHeartbeat('brokkr', INSTANCE_A, '2026-07-07')) },
      { instanceId: INSTANCE_A, day: '2026-07-09', payload: JSON.stringify(makeHeartbeat('brokkr', INSTANCE_A, '2026-07-09')) },
      { instanceId: INSTANCE_B, day: '2026-07-08', payload: JSON.stringify(makeHeartbeat('brokkr', INSTANCE_B, '2026-07-08')) },
    ]);
  ...
  test('an instance whose heartbeats have all been pruned keeps a NULL first-report day', () => {
    ...
    try {
      ...
      db.recordHeartbeat(INSTANCE_A, LEGACY_PRODUCT, '2026-08-01', 1, '{}', '2026-08-01');
      expect(db.getInstance(INSTANCE_A, LEGACY_PRODUCT)?.first_report_day).toBeNull();
    } finally {
      db.close();
    }
  ...
  test('a table declared with different column CASING is recognised, not rebuilt and overwritten', () => {
    ...
    raw
      ...
      .run(INSTANCE_A, 'saga', '2026-07-07', 1, JSON.stringify({ schema_version: 1, metrics: {} }));
    raw.close();
  ...
  test('a LEGACY table declared with different casing is still found and rebuilt', () => {
    ...
    raw
      ...
      .run(INSTANCE_A, '2026-07-07', 1, JSON.stringify(makeHeartbeat('brokkr', INSTANCE_A, '2026-07-07')));
    raw.close();
  ...
  test('a WITHOUT ROWID heartbeats table does not break the migration', () => {
    // It passes every shape check and so is not rebuilt. Anything in the
    // migration that assumed `rowid` would fail here, roll the whole
    // transaction back, and leave the service unable to start.
    const path = tempDbPath();
    const raw = new Database(path, { create: true });
    raw.exec(`
      CREATE TABLE heartbeats (
    ...
    for (const day of ['2026-07-07', '2026-07-08']) {
      insert.run(INSTANCE_A, 'brokkr', day, 1, JSON.stringify(makeHeartbeat('brokkr', INSTANCE_A, day)));
    }
  ...
  test('the rebuild carries a large table across intact, every row and every payload', () => {
    const path = tempDbPath();
    const rows: SeedRow[] = [];
    const expected = new Map<string, string>();
    for (let i = 0; i < 1200; i++) {
      const day = new Date(Date.UTC(2026, 0, 1) + i * 86_400_000).toISOString().slice(0, 10);
      const payload = JSON.stringify(makeHeartbeat('brokkr', INSTANCE_A, day));
      expected.set(day, payload);
      rows.push({ instanceId: INSTANCE_A, day, payload });
    }
    seedOldDatabase(path, rows);
    ...
    try {
      ...
      const carried = db.getHeartbeats(INSTANCE_A, LEGACY_PRODUCT);
      expect(carried.length).toBe(1200);
      for (const row of carried) expect(row.payload).toBe(expected.get(row.day)!);
    } finally {
      db.close();
    }
  });
