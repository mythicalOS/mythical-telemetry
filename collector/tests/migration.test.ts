// The /v1 -> /api/v1 + /healthz -> /health migration gate (API-M3 criteria 2 and 3).
//
// Spec-then-migrate, step 2's proof. PRE_MIGRATION_ROUTES is the router's inventory FROZEN as it
// was before the prefix change (see the commit that specced the pre-migration router). Two gates
// run against it:
//   criterion 2 (drop-nothing): every pre-migration route has a live post-migration counterpart
//     modulo the prefix — nothing was dropped in the rename — and the ONLY routes added are the
//     two deliberate convention routes (HEAD /health, GET /openapi.json).
//   criterion 3 (no aliases / hard cut): every RENAMED old path now 404s. There is no dual-serving
//     window — POST /v1/ingest is gone, POST /api/v1/ingest answers.
