// Machine-first guard on the telemetry responses.
//
// A GUARD, not a did-nothing detector: a service with no live offender passes. It asserts the
// /api/v1 + /health surfaces stay machine-consumable — no capability route returns an
// HTML body (on ERROR *or* authenticated SUCCESS), numeric value fields are unit-suffixed, and
// error tokens are lowercase tokens, not prose. A newly introduced machine-hostile field on any
// of those surfaces would fail this.
