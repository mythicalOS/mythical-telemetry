...
describe('operator artifacts are present', () => {
  ...
  test('the published OpenAPI document does not claim anonymity either', () => {
    // The guard above pinned the README and stopped there, so the same claim
    // survived for months in the document that is served at /openapi.json AND
    // rendered verbatim onto the public documentation site's reference page —
    // three times, including the ingest summary. A rule that covers one file and
    // not the other reads as enforced while the wording ships anyway, which is
    // worse than no rule: the passing test is what stops anyone looking.
    //
    // FIELDS.md states the requirement without naming a file: never describe
    // the data as anonymous in UI copy, README text, or the privacy notice. An
    // API description that is published to readers is all three.
    const spec = readFileSync(join(ROOT, 'api', 'openapi.json'), 'utf8');
    for (const claim of [
      /\b(?:is|are|it's)\s+anonymous\b/i,
      /\banonymous\s+(?:daily|product|usage|telemetry|heartbeat)/i,
    ]) {
      expect(claim.test(spec), `the OpenAPI document claims anonymity: ${claim}`).toBe(false);
    }
  });
