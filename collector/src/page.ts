// The public give-back page — AGGREGATE ONLY.
//
// THERE IS NO PER-INSTALL PAGE, AND WHY: an unauthenticated `/i/<uuid>` route whose
// entire access control is "you know the UUID" would make the instance id a bearer
// read capability, which it is not. An unauthenticated per-install surface beside a
// non-bearer id would be a contradiction, and a route that renders one install's
// history to whoever can produce its id is exactly the capability this service
// withholds. Per-install reads happen over the authenticated JSON API.
//
