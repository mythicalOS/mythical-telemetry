...
//   GET    /api/v1/instances/:uuid/stats   secret + derived-identity (reads are authenticated)
...
//   GET    /health, HEAD /health           200 {ok, version, uptime_s} — liveness
//   GET    /openapi.json                   the committed OpenAPI document, served verbatim
//
// The versioned surface is /api/v1/* and the liveness route is /health; the unversioned /v1/*
// and /healthz paths are not served (no aliases — they 404). The Worker twin shares one document; a parity test binds them.
//
// There is no per-install HTML page (`GET /i/<uuid>` is not a route) — see page.ts for why
// such a route cannot exist here.
//
// AUTHORIZATION MODEL. The instance id is not a bearer read
...
// The committed OpenAPI document, served verbatim at GET /openapi.json. Both
// this collector and the Worker twin import the SAME file, so they serve identical bytes — the
// route-parity test and the byte-equality test in api/openapi.test.ts bind that.
...
export interface TelemetryServerConfig {
  ...
  /** Reported by GET /health (the liveness triple). Defaults to '0.0.0'. */
  version?: string;
...
/**
 * The COLLECTOR's route inventory — the single source of truth for which routes exist, and the
 * table the dispatcher below is driven by (not a parallel if-chain). Three consumers read it:
 * the dispatcher (to match), the conformance suite (to enumerate), and the
 * route-parity test, which asserts the Worker twin's own independent copy
 * (`collector-worker/src/server.ts`) is set-equal to this one — so a route added to one collector
 * and not the other fails. `{id}` is the one path parameter (an instance UUID).
 */
...
export function buildFetchHandler(config: TelemetryServerConfig): FetchHandler {
  ...
  /**
   * GET /api/v1/instances/:uuid/stats?product=… — AUTHENTICATED.
   *
  ...
  const routeHandlers: Record<string, RouteHandler> = {
    // Liveness triple. Nothing here needs protecting, so it stays unauthenticated.
    'GET /health': () =>
      json(200, { ok: true, version, uptime_s: Math.floor((nowMs() - startedAtMs) / 1000) }),
    // HEAD answers with the same status and headers and an empty body.
    'HEAD /health': () => new Response(null, { status: 200, headers: { 'content-type': 'application/json' } }),
    // The served document IS the committed one (imported above).
    'GET /openapi.json': () =>
  ...
  if (
    ...
    throw new Error('collector route table drift: ROUTES and routeHandlers disagree');
  }
